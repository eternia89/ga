...
function loadConfig(cwd) {
  ...
  try {
    ...
    if ("depth" in parsed && !("granularity" in parsed)) {
      ...
      parsed.granularity =
        depthToGranularity[parsed.depth] || parsed.depth;
      delete parsed.depth;
      try {
        fs.writeFileSync(
          configPath,
  ...
  try {
    ...
    execSync(
      "git check-ignore -q --no-index -- " +
        targetPath.replace(/[^a-zA-Z0-9._\-/]/g, ""),
      {
        cwd,
        stdio: "pipe",
      },
...
function findPhaseInternal(cwd, phase) {
  ...
  try {
    ...
    const archiveDirs = milestoneEntries
      .filter(
        (e) => e.isDirectory() && /^v[\d.]+-phases$/.test(e.name),
      )
      .map((e) => e.name)
      .sort()
      .reverse();
