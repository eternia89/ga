...
- **Global scope**: `/Users/melfice/code/ga/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/melfice/code/ga/.claude/notes/`)
...
2. Glob `/Users/melfice/code/ga/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/melfice/code/ga/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/melfice/code/ga/.claude/notes/` — works in any directory
