...
export default function ScheduleDetailError({
  ...
  useEffect(() => {
    console.error('[ScheduleDetailError]', error);
  }, [error]);

  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] px-4 text-center">
      <CalendarClock className="h-10 w-10 text-amber-400 mb-4" />
      <h2 className="text-xl font-semibold text-foreground mb-2">
        Could not load schedule
      </h2>
      <p className="text-sm text-muted-foreground mb-1 max-w-sm">
        There was a problem fetching this maintenance schedule. This may be a
